...
- `plan_worker: codex` (JSON `planWorker`) routes PLAN to Codex ACP. YAML `worker: hybrid` / JSON `provider: "hybrid"` with no `plan_worker` is the same shorthand; explicit `plan_worker` wins. IMPL stays Grok. Unknown `plan_worker` is `unknown_plan_worker` at admit.
